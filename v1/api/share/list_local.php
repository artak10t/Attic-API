...
try{
  try{
    ...
    $sort_by = "rcpt"; // default sort column
    ...
    $filter_defs = array(
      array("share_id", "?", FILTER_INT),
      array("rcpt", "?", FILTER_STR),
    ...
    $sql = "SELECT bin_to_uuid(`account_id`) AS `account_id`, `account_name`, `share_id`, `rcpt`, bin_to_token(`token`) AS `token` FROM `v_local_shares` WHERE `account_id` = uuid_to_bin(?)";
    $ctrl_sql = "SELECT COUNT(0) FROM (SELECT 0 FROM `v_local_shares` WHERE `account_id` = uuid_to_bin(?)";
    if ($expression != ""){
      $sql .= " AND ".$expression;
    ...
    do{
      ...
      while($arr = $result->fetch_assoc()){
        ...
        $shares["share_id"] = $arr["share_id"];
        $shares["rcpt"] = $arr["rcpt"];

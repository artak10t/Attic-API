...
try{
  try{
    ...
    do{
      ...
      $stmt = $mysql->query('SELECT `disabled`, `reason` FROM `v_sessions` WHERE `session_id` = token_to_bin(?)', array("s"), array($session_id));
    ...
    $sql = "SELECT bin_to_uuid(`account_id`) AS `account_id`, `name` FROM `accounts` WHERE `disabled` = 0";

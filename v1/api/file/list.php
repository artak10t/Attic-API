...
try{
  try{
    ...
    do{
      ...
      // check if folder_id is valid, and owned by the same user
      $stmt = $mysql->query('SELECT bin_to_uuid(`account_id`) AS `account_id` FROM `folders` WHERE `folder_id` = uuid_to_bin(?)', array("s"), array($folder_id));
      if ($mysql->deadlock()) continue;
      $result = $stmt->get_result();
      if (!$arr = $result->fetch_assoc()) throw new Exception("Folder doesn't exist", E_DOESNT_EXIST);
      if ($arr["account_id"] != $account_id) throw new Exception("Folder is owned by another account", E_UNAUTHORIZED);
      $stmt->close();

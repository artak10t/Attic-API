  ...
  try{
    ...
    do{
      ...
      if (!is_null($parent_id)){
        // check if parent_id is valid, and owned by the same user
        $stmt = $mysql->query('SELECT bin_to_uuid(`account_id`) AS `account_id` FROM `folders` WHERE `folder_id` = uuid_to_bin(?)', array("s"), array($parent_id));
        if ($mysql->deadlock()) continue;
        $result = $stmt->get_result();

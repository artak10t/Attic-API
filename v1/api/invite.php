...
try{
  try{
    ...
    do{
      ...
      $stmt = $mysql->query('SELECT bin_to_uuid(`account_uid`) FROM `sessions` WHERE `session_id` = token_to_bin(?) LOCK IN SHARE MODE', array("s"), array($session_id));
      ...
      // check if user is admin and account is enabled
      $stmt = $mysql->query('SELECT `admin`, `disabled`, `reason` FROM `accounts` WHERE `uid` = uuid_to_bin(?) LOCK IN SHARE MODE', array("s"), array($account_uid));
      if ($mysql->deadlock()) continue;
      $result = $stmt->get_result();
      ...
      $stmt = $mysql->query('INSERT INTO `tokens`(`account_uid`, `token`, `type`) VALUES (uuid_to_bin(?), token_to_bin(?), 1)', array("s", "s"), array($account_uid, $token));
      if ($mysql->deadlock()) continue;
      $stmt->close();

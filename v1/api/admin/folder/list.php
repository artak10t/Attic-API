...
try{
  try{
    ...
    $foreign_account_id = null;
    if(isset($post['foreign_account_id'])){
    ...
    // Both foreign_account_id and parent_id fields cannot be NULL
    // If parent_id is specifed, foreign_account_id will be ignored
    if (is_null($foreign_account_id) && is_null($parent_id)) throw new Exception("Both foreign_account_id and parent_id fields are NULL", E_FIELD_INVALID);
    ...
    if (is_null($parent_id)){
      $sql = "SELECT bin_to_uuid(`account_id`) AS `account_id`, `account_name`, bin_to_uuid(`parent_id`) AS `parent_id`, `parent_name`, ".
              "bin_to_uuid(`folder_id`) AS `folder_id`, `name` FROM `v_folders` WHERE `account_id` = uuid_to_bin(?) AND `parent_id` IS NULL";
      $ctrl_sql = "SELECT COUNT(0) FROM (SELECT 0 FROM `v_folders` WHERE `account_id` = uuid_to_bin(?) AND `parent_id` IS NULL";
      array_unshift($types, "s");
      array_unshift($values, $foreign_account_id);
    }else{
      $sql = "SELECT bin_to_uuid(`account_id`) AS `account_id`, `account_name`, bin_to_uuid(`parent_id`) AS `parent_id`, `parent_name`, ".
              "bin_to_uuid(`folder_id`) AS `folder_id`, `name` FROM `v_folders` WHERE `parent_id` = uuid_to_bin(?)";
      $ctrl_sql = "SELECT COUNT(0) FROM (SELECT 0 FROM `v_folders` WHERE `parent_id` = uuid_to_bin(?)";
      array_unshift($types, "s");
      array_unshift($values, $parent_id);
    }
    if ($expression != ''){
      $sql .= " AND ".$expression;
    ...
    do{
      ...
      while($arr = $result->fetch_assoc()){
        $folders = array();
        $folders["account_id"] = $arr["account_id"];
        $folders["account_name"] = $arr["account_name"];
        $folders["parent_id"] = $arr["parent_id"];
        $folders["parent_name"] = $arr["parent_name"];
        $folders["folder_id"] = $arr["folder_id"];
        $folders["name"] = $arr["name"];

        array_push($data["folders"], $folders);
      }
      $stmt->close();
    } while ($mysql->deadlock());
    $mysql->commit();

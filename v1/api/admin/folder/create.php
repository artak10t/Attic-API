  ...
  try{
    ...
    do{
      ...
      if (!is_null($parent_id)){
        ...
        if ($arr["account_id"] != $foreign_account_id) throw new Exception("Parent folder is owned by another account", E_UNAUTHORIZED);
        $stmt->close();
      }
      ...
      $stmt = $mysql->query('INSERT INTO `folders`(`account_id`, `folder_id`, `parent_id`, `name`) VALUES (uuid_to_bin(?), uuid_to_bin(?), uuid_to_bin(?), ?)',
                            array("s", "s", "s", "s"), array($foreign_account_id, $uuid, $parent_id, $name));
      if ($mysql->deadlock()) continue;
      $stmt->close();
    } while ($mysql->deadlock());
    $mysql->commit();

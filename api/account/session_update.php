...
try
{
  if($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['help'])) header('Location: '."/help/account/session_update.html");
  ...
  MySQL::query('UPDATE `sessions` SET `updated` = UTC_TIMESTAMP() WHERE `session_id` = token_to_bin(?)', array("s"), array($session_id));
  MySQL::stmt_close();
  MySQL::commit();

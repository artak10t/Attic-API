...
try
{
  if($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['help'])) header('Location: '."/help/account/delete_cancel.html");
  ...
  MySQL::query('SELECT `enabled` FROM `accounts` WHERE `account_id` = (SELECT `account_id` FROM `sessions` WHERE `session_id` = token_to_bin(?))', array("s"), array($session_id));
  ...
  MySQL::query('UPDATE `accounts` SET `delete_on` = NULL WHERE `account_id` = (SELECT `account_id` FROM `sessions` WHERE `session_id` = token_to_bin(?))', array("s"), array($session_id));
  MySQL::stmt_close();
  MySQL::commit();

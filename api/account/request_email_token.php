...
try
{
  if($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['help'])) header('Location: '."/help/account/request_email_token.html");
  ...
  MySQL::query('INSERT INTO `tokens`(`account_id`, `token_bin`, `token_type`)
                VALUES ((SELECT `account_id` FROM `sessions` WHERE `session_id` = token_to_bin(?)), token_to_bin(?), 3)', array("s", "s"), array($session_id, $token));
  ...
  $message = file_get_contents("$root/config/forms/email/account_email_change.html");
  $prepend = $prepend.$token;
  $message = str_replace("%link%", $prepend, $message);

  if(!mail($email, "Email Change", $message, implode("\r\n", $headers)))

...
function change_email($token, $email)
{
  MySQL::connect();
  MySQL::start_transaction();

  MySQL::query('SELECT `account_id` FROM `tokens` WHERE `token_bin` = token_to_bin(?) AND `token_type` = 3', array("s"), array($token));
  ...
  MySQL::query('UPDATE `accounts` SET `email` = ? WHERE `account_id` = ?', array("s", "i"), array($email, $account_id));
...
function change_email_foreign($session_id, $account_id, $email)
{
  MySQL::connect();
  MySQL::start_transaction();
